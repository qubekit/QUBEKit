...
from QUBEKit.fitting import ForceBalanceFitting, Priors, TorsionProfile
...
def test_torsion_target_generation(tmpdir, biphenyl):
    ...
    with tmpdir.as_cwd():
        ...
        # load the xyz file and make sure we have 24 geometries
        biphenyl.add_conformers(
            file_name=os.path.join(target_name, "scan.xyz"), input_type="temp"
        )
        assert len(biphenyl.coords["temp"]) == 24
...
def test_full_optimise(tmpdir, biphenyl):
    ...
    with tmpdir.as_cwd():
        ...
        # use default values
        fb = ForceBalanceFitting()
        fitted_molecule = fb.optimise(molecule=biphenyl)
        # now compare the fitted results
        for torsion, old_values in biphenyl.PeriodicTorsionForce.items():
            central_bond = torsion[1:3]
            if central_bond == [10, 11] or central_bond == [11, 10]:
                # compare the k values, making sure they have been changed
                fitted_values = fitted_molecule.PeriodicTorsionForce[torsion]
                for i in range(5):
                    assert old_values[i][1] != fitted_values[i][1]
...
def test_qdata_round_trip(tmpdir):
    ...
    with tmpdir.as_cwd():
        ...
        # now write to file
        td_ref.create_qdata(file_name="ref.txt")
        # now make a second object
        td_new = TorsionDriveData.from_qdata(
            dihedral=(6, 10, 11, 8), qdata_file="ref.txt"
        )
        ...
        for angle, ref_result in td_ref.reference_data.items():
            ...
            assert ref_result.energy == new_result.energy
            assert np.allclose(
