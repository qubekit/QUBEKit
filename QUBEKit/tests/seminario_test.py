...
def test_mod_sem_no_symmetry(tmpdir):
    ...
    with tmpdir.as_cwd():
        ...
        assert mol.BondForce[(0, 1)].length == pytest.approx(
            0.15344171531887932
        )
        assert mol.BondForce[(0, 1)].k == pytest.approx(
            192462.76956156612
        )
        assert mol.BondForce[(0, 2)].length == pytest.approx(
            0.10954907576059233
        )
        assert mol.BondForce[(0, 2)].k == pytest.approx(295645.6124892813)

        assert mol.AngleForce[(1, 0, 2)].angle == pytest.approx(
            1.9423960113296368
        )
        assert mol.AngleForce[(1, 0, 2)].k == pytest.approx(
            374.76469990519263
        )
        assert mol.AngleForce[(1, 0, 3)].k == pytest.approx(
            401.56353614024914
        )
        assert mol.AngleForce[(1, 0, 4)].angle == pytest.approx(
            1.9416241741805782
        )
        assert mol.AngleForce[(1, 0, 3)].angle == pytest.approx(
            1.9422108316309619
        )
        assert mol.AngleForce[(1, 0, 4)].k == pytest.approx(
            371.0717571027322
        )
        assert mol.AngleForce[(2, 0, 3)].angle == pytest.approx(
            1.8787818480998344
        )
        assert mol.AngleForce[(2, 0, 3)].k == pytest.approx(
            314.5677633711689
        )
        assert mol.AngleForce[(0, 1, 6)].angle == pytest.approx(
            1.9423960113296368
        )
        assert mol.AngleForce[(0, 1, 6)].k == pytest.approx(
            399.59297576081184
        )
...
def test_mod_sem_symmetry(tmpdir):
    ...
    with tmpdir.as_cwd():
        ...
        for angles in mol.angle_types.values():
            values = set()
            for angle in angles:
                values.add(tuple([mol.AngleForce[angle].angle, mol.AngleForce[angle].k]))
                assert len(values) == 1
...
def test_hessian_unit_regression(tmpdir):
    ...
    with tmpdir.as_cwd():
        ...
        assert round(mol.AngleForce[(1, 0, 2)].angle * constants.RAD_TO_DEG, 2) == 108.50
        assert round(mol.AngleForce[(1, 0, 2)].k * constants.KJ_TO_KCAL / 2, 2) == 38.16

        # check a H-C-H angle
        assert round(mol.AngleForce[(2, 0, 3)].angle * constants.RAD_TO_DEG, 2) == 110.31
