...
class TorsionProfile(TargetBase):
    ...
    def prep_for_fitting(self, molecule: Ligand) -> List[str]:
        ...
        for scan in molecule.qm_scans:
            ...
            molecule.write_pdb(input_type="input", name="molecule")
            # write the qdata file
            scan.create_qdata()
            # prep the scan.xyz file
            molecule.coords["traj"] = []
            for data in scan.reference_data.values():
                molecule.coords["traj"].append(data.geometry)
            molecule.write_xyz(input_type="traj", name="scan")
            # make the metadata
...
class ForceBalanceFitting(BaseModel):
    """
    ...
    def optimise(self, molecule: Ligand) -> Ligand:
        """
    ...
    def generate_forcefield(molecule: Ligand) -> None:
        ...
        tags.PeriodicTorsionForce_tags = {"parameterize": "k1, k2, k3, k4"}
        ...
        for torsiondrive_data in molecule.qm_scans:
            ...
            tags.PeriodicTorsionForce_groups.extend(dihedrals)

        # now we have all of the dihedrals build the force field
        molecule.write_parameters(
            name=os.path.join("forcefield", "bespoke"), parameter_tags=tags
        )
    ...
    def collect_results(self, molecule: Ligand) -> Ligand:
        ...
        if not status:
            ...
            # TODO change to extract the topology and positions from the ligand
            molecule.write_pdb()
            XML(
                molecule=molecule_copy,
                input_file=os.path.join("result", self.job_type, "bespoke.xml"),
            )
