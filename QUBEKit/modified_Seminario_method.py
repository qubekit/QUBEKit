def modified_Seminario_method(molecule, vibrational_scaling):
    ...
    bonds = list(molecule.topology.edges)
    bond_list = []
    for i in range(len(bonds)):
        bond_list.append([bonds[i][0]-1, bonds[i][1]-1])
    print(bond_list)
    angles = list(molecule.angles)
    angle_list = []
    for i in range(len(angles)):
        angle_list.append([angles[i][0]-1, angles[i][1]-1, angles[i][2]-1])
    hessian = molecule.hessian
    atom_names = molecule.atom_names
    N = len(molecule.QMoptimized)

    print(bond_list, angle_list)

    coords = []
    for i in range(len(molecule.QMoptimized)):
        for j in range(3):
            coords.append(molecule.QMoptimized[i][j + 1])
    #coords = [0.0, 0.764, 0.0, 0.0, -0.764, 0.0, 1.019, 1.158, 0.0, -0.51, 1.158, 0.883,-0.51, 1.158, -0.883,-1.019, -1.158, 0.0, 0.51, -1.158, -0.883, 0.51, -1.158, 0.883]
...
def input_data_processing():
    ...
    [bond_list, angle_list] = bond_angle_list()
    ...
    if os.path.exists('Zmat.z'):
        atom_names = []
        
        fid = open( 'Zmat.z') #Boss type Zmat
...
def sb_file_new_parameters(filename):
#Takes new angle and bond terms and puts them into a .sb file with name
#filename_seminario.sb

    fid_angles = open( 'Average_Modified_Seminario_Angles', 'r')
    fid_bonds = open('Average_Modified_Seminario_Bonds', 'r')
    ...
    #Opens Files
    fidout = open( filename  + '_Seminario.sb', 'wt') #Script produces this file
